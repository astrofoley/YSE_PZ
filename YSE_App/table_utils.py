...
from .query_annotations import (
    annotate_recent_mag,
    annotate_recent_magdate,
    annotate_transient_search_fields,
)
rcParams['figure.figsize'] = (7,7)


def _order_transient_recent_mag(queryset, is_descending, transient_id_field="pk"):
...
def _order_transient_recent_magdate(queryset, is_descending, transient_id_field="pk"):
    queryset = annotate_recent_magdate(
        queryset,
...
def annotate_with_disc_mag(qs):
    return annotate_transient_search_fields(qs)
...
def _bind_recent_mag_ordering(table_cls, transient_id_field, include_recent_magdate=True):
    def order_recent_mag(self, queryset, is_descending):
        return _order_transient_recent_mag(
            queryset,
            is_descending,
            transient_id_field=transient_id_field,
        )

    table_cls.order_recent_mag = order_recent_mag

    if include_recent_magdate:
        def order_recent_magdate(self, queryset, is_descending):
            return _order_transient_recent_magdate(
                queryset,
                is_descending,
                transient_id_field=transient_id_field,
            )

        table_cls.order_recent_magdate = order_recent_magdate


# Keep the hot table sorts on one shared implementation so every transient list
# benefits from the same indexed subquery path.
for _table_cls in (
    TransientTable,
    FieldTransientTable,
    ...
    _bind_recent_mag_ordering(_table_cls, transient_id_field="pk")

for _table_cls in (FollowupTable, ObsNightFollowupTable, ToOFollowupTable):
    _bind_recent_mag_ordering(
        _table_cls,
        transient_id_field="transient_id",
        include_recent_magdate=False,
    )
