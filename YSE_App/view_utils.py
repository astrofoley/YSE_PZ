...
def get_recent_phot_for_host(user, host_id=None):
    photdata = PhotometryService.GetAuthorizedHostPhotData_ByUser_ByHost(
        user,
        host_id,
        includeBadData=True,
    )
    return photdata.order_by('-obs_date').first()
...
def get_disc_mag_for_transient(user, transient_id=None):
    ...
    discovery_phot = photdata.filter(discovery_point=True, mag__isnull=False).first()
    if discovery_phot:
        return discovery_phot

    return photdata.filter(mag__isnull=False).first()
