...
from .query_annotations import annotate_days_since_disc, annotate_recent_mag


def _first_prefetched_survey_field(msb):
    survey_fields = list(msb.survey_fields.all())
    return survey_fields[0] if survey_fields else None


def _survey_observation_mjds(msb_lists):
    field_ids = []
    for msb_list in msb_lists:
        for msb in msb_list:
            survey_field = _first_prefetched_survey_field(msb)
            if survey_field is not None:
                field_ids.append(survey_field.id)

    obs_mjds_by_field_id = defaultdict(list)
    if not field_ids:
        return obs_mjds_by_field_id

    for survey_field_id, obs_mjd in SurveyObservation.objects.filter(
        survey_field_id__in=field_ids,
        obs_mjd__isnull=False,
    ).order_by('survey_field_id', '-obs_mjd').values_list('survey_field_id', 'obs_mjd'):
        obs_mjds_by_field_id[survey_field_id].append(obs_mjd)
...
def select_yse_fields(request):
    ...
    all_yse_gpc1_fields = SurveyFieldMSB.objects.filter(survey_fields__obs_group__name='YSE').filter(survey_fields__instrument__name='GPC1').distinct().order_by('name').prefetch_related('survey_fields')
    active_yse_gpc1_fields = SurveyFieldMSB.objects.filter(survey_fields__obs_group__name='YSE').filter(survey_fields__instrument__name='GPC1').filter(active=True).distinct().order_by('name').prefetch_related('survey_fields')
    all_yse_gpc2_fields = SurveyFieldMSB.objects.filter(survey_fields__obs_group__name='YSE').filter(survey_fields__instrument__name='GPC2').distinct().order_by('name').prefetch_related('survey_fields')
    active_yse_gpc2_fields = SurveyFieldMSB.objects.filter(survey_fields__obs_group__name='YSE').filter(survey_fields__instrument__name='GPC2').filter(active=True).distinct().order_by('name').prefetch_related('survey_fields')

    active_gpc1_names = set(active_yse_gpc1_fields.values_list('name',flat=True))
    active_gpc2_names = set(active_yse_gpc2_fields.values_list('name',flat=True))
    obs_mjds_by_field_id = _survey_observation_mjds(
        (
            all_yse_gpc1_fields,
            active_yse_gpc1_fields,
            all_yse_gpc2_fields,
            active_yse_gpc2_fields,
        )
    )
    ...
    for a in all_yse_gpc1_fields:
        ...
        survey_field = _first_prefetched_survey_field(a)
        if survey_field is None:
            continue
        obs_mjds = obs_mjds_by_field_id[survey_field.id]
        if obs_mjds:
            last_obs_date = obs_mjds[0]
    ...
    for a in active_yse_gpc1_fields:
        ...
        if survey_field is None:
            continue
        obs_mjd = np.sort(np.array(obs_mjds_by_field_id[survey_field.id]))
        if len(obs_mjd):
...
def return_serialized_transients(request):
    ...
    if len(request.META['QUERY_STRING'].replace('q=','')) >= 5:
        ...
        response = list(transients.values("id", "name")[:20])
...
def yse_sky(request):
    ...
    transients = annotate_recent_mag(transients)

    transients = annotate_days_since_disc(transients).order_by('-days_since_disc')
...
def yse_planning(request):
    ...
    rising_transients = annotate_recent_mag(rising_transients)

    rising_transients = annotate_days_since_disc(rising_transients)
    ...
    good_transients = annotate_recent_mag(good_transients).filter(~Q(tags__name='YSE'))

    good_transients = annotate_days_since_disc(good_transients)
    ...
    nearby_transients = annotate_recent_mag(nearby_transients).filter(~Q(tags__name='YSE'))

    nearby_transients = annotate_days_since_disc(nearby_transients)
...
def yse_fields(request,ra_min_hour,ra_max_hour,min_mag):
    ...
    for s in ztfsurveyfields:
        ...
        qs_final = annotate_recent_mag(qs_final)
        ...
        qs_final = annotate_days_since_disc(qs_final)
